...
# 1. EPİAŞ Veri Çekme (Hata durumunda otomatik Mock Data üretir)
def get_epias_prices():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            raise Exception("API Hata")
    except:
        # Hata durumunda sistemin durmaması için gerçekçi fiyatlar (Mock Data)
        st.warning("Canlı veri alınamadı, test verileri kullanılıyor.")
        hours = range(24)
        # Sabah düşük, akşam yüksek (gerçekçi fiyat senaryosu)
        prices = [500 + (100 * np.sin(h/24 * 2 * np.pi) + np.random.randint(0, 100)) for h in hours]
        return pd.DataFrame({'date': [f"{h}:00" for h in hours], 'price': prices})
